...
- [x] **Starter-aware Explore suggestion** — if latest starter log is <6hrs old, shows "Your starter is active — ready to bake!" banner surfacing long-ferment recipes.
...
- [x] **Recipe social proof** — bake count + avg community rating on recipe detail page. "X bakers have made this · ★ Y.Y".
- [x] **Starter guide Day 13 → first bake CTA** — "You're ready — let's bake your first loaf →" deep-links to Classic Artisan Sourdough.
- [x] **Troubleshooter → journal link** — TroubleshootDetail shows "Log this in your Bake Journal →" linking to `/journal/new`.

**Personalization features:**
- [x] **Lightweight Recipe Fork** — "✏️ My Version" button on recipe detail. Copies to user account with editable title, description, ingredient amounts. Lineage "Based on [Original]" preserved. Steps copied as-is. Private by default. Migration 012 added `forked_from_recipe_id` column. My Recipes tab added to Explore page (My Versions + Saved sections).
...
| 2026-03-11 | Flow fixes + Recipe Fork shipped (v0.4) | Starter-aware Explore suggestion wired (6hr starter freshness check → long-ferment banner). Recipe social proof: bake count + avg rating on detail page. Starter guide Day 13 CTA deep-links to Classic Artisan Sourdough. Troubleshooter detail page links to journal. Recipe Fork (migration 012: forked_from_recipe_id): "✏️ My Version" button on detail, `/recipes/:id/fork` protected route, editable title/desc/ingredient amounts, lineage display. My Recipes tab on Explore: My Versions (user-owned) + Saved (favorites) sections. Auth gate on fork with contextual modal. All user-facing fork copy de-jargoned ("Make It Yours" vs "Fork Recipe"). |
| 2026-03-11 | UX audit stability pass (v0.4) | FavoriteButton tap target: w-10/w-8 → w-11 (both sizes, WCAG AA 44px minimum). inputMode="decimal" added to all number inputs in StarterForm (1) and FeedingLogForm (5) — prevents iOS numeric keyboard from showing phone-style layout. ProfileEdit toggle: w-11 h-6 → w-12 h-7, thumb w-5 h-5 → w-6 h-6 (toggle thumb was clipping container). |
